...
function Trending(props) {
 ...
 const trending_url=`https://api.themoviedb.org/3/trending/movie/${timeData}?api_key=193e9601f03e6c6ba83956dd0f6de6af`;
  useEffect(() => {
    fetch(trending_url)
  ...
  return (
    <> <div>
      <h1 className='offset-1 mb-2'>Tendencia
        <Box
          sx={{
            display: 'inline',
        ...
        >
          <ButtonGroup size="medium" aria-label="medium button group" className='offset-1'>
            <Button key="today" onClick={handleDayButton}>Hoy</Button>
            <Button key="lastweek" onClick={handleWeekButton}>Última semana</Button>
          </ButtonGroup>
        </Box>
      </h1>
      </div>
